...
class LibraryPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        for combo in (self.year_filter, self.rating_filter, self.artist_filter, self.participant_count_filter, self.participant_filter, self.genre_filter, self.region_filter):
            combo.activated.connect(lambda _=0: self.filters_changed.emit())
    ...
    def set_albums(self, albums):
        # Не отключаем перерисовку страницы во время перестройки сетки.
        # На некоторых оконных системах это может оставлять старый backing store
        # до следующего полного repaint и проявляться как краткий "микро-окно".
        while self.grid.count():
            item = self.grid.takeAt(0)
            widget = item.widget()
            if widget:
                widget.setVisible(False)
                stop = getattr(widget, "stop_animations", None)
                if stop:
                    stop()
                # Не отвязываем старую карточку от контейнера: setParent(None)
                # превращает QWidget во временный top-level объект.
                widget.deleteLater()
        ...
        for index, row in enumerate(albums):
            ...
            card = ImageTileCard(
                ...
                "album", parent=self.container, rating=rating
            )
